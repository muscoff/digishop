...
if(isset($_POST['first_cap'])){
	...
	if(isset($_FILES['image']['name']) & !empty($_FILES['image']['name'])){
		if($_FILES['image']['size'] <= $max_size){
			if(in_array($_FILES['image']['type'], $imageStack)){
				...
				$data = $page->fetch();
				$imageLocal = $data['image'];
				$explodeImageLocal = explode('/', $imageLocal);
				$count = count($explodeImageLocal);
				$imageLocal = $explodeImageLocal[$count-1];

				if(move_uploaded_file($tmp, $serverName)){
					if($page->edit()){
						unlink($server.$imageLocal);
	...
	}//End of checking if image has been uploaded
	else{
