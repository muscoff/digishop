...
<div class="font-allerBd uppercase center-text margin-top-10">Seventh section</div>
	...
	<div class="container">
		<div class="row">
			<div class="col-6 col-s-12 padding-all-10 relative">
				<form method="POST" enctype="multipart/form-data" action="<?=url_location;?>api/pages/page_seven/<?=(($count>0)?'edit':'create');?>.php">
					...
					<div class="margin-top-10">
						...
						<input type="text" name="first_cap" value="<?=((!empty($data['first_cap']))?$data['first_cap']:'');?>"><br />
					</div>

					<div class="margin-top-10">
						<div class="uppercase font-allerRg">second caption</div>
						<input type="text" name="second_cap" value="<?=((!empty($data['second_cap']))?$data['second_cap']:'');?>"><br />
					</div>

					<div class="margin-top-10">
						<div class="uppercase font-allerRg">third caption</div>
						<input type="text" name="third_cap" value="<?=((!empty($data['third_cap']))?$data['third_cap']:'');?>"><br />
					</div>
						...
						<div class="col-12 col-s-12 padding-all-10">
							...
							<select name="page_link">
								<?php foreach($first as $first): ?>
									<option value="<?=$first['id'];?>" <?=(($count>0 & $first['id'] == $data['page_link'])?'selected':'');?> ><?=$first['category'];?></option>
								<?php endforeach; ?>
							</select>
						</div>
					</div>
				...
				<?php if($count>0): ?>
					<div class="absolute right-0 bottom-0">
						<a href="<?=url_location;?>api/pages/page_seven/delete.php?id=<?=$data['id'];?>">
...
<div class="width-100 height-100 height-s-50 overflow-hidden margin-top-20">
	...
	<div class="width-100 height-100 margin-top-20">
		<div class="full iframe">
			<iframe src="/digishop/pages/sectionseven.php"></iframe>
		</div>
	</div>
</div>
